 ...
 */
export default async function allianceSubscription(this: Kit, event: subscriptionEvent, callback: Function, filters?: allianceSubscriptionFilters) {
    ...
    const channelName = JSON.parse(await (await fetch(`https://api.politicsandwar.com/subscriptions/v1/subscribe/alliance/${event}?api_key=${this.apiKey}${linkFilter}`, {
        method: 'GET',
    })).text()).channel;
    ...
    channel.bind(`BULK_ALLIANCE_${event.toUpperCase()}`, function (data: alliance[]) {
